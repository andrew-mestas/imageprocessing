...
// White:
    ...
    var myFunc = gpu.createKernel(function(A, B, C, D, E) {
       for(var x=0; x< C; x++){     
        ...
        for(var i=0; i < D; i++){
          ...
          for(var b=0; b<3;b++){
            similarity += Math.pow(E[i][b] - B[i][b], 2);
      ...
      //  for(var x=0; x< C; x++){     
      //  }
      //   return this.thread.x + A[this.thread.x] + 2;

    }, {dimensions: [dim], loopMaxIterations: ln});

    var r = myFunc(0, array, ln, colorLength, Color)
    console.log(r)
